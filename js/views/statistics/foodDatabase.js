...
export const FoodDatabaseView = {
    ...
    loadPreferences() {
        ...
        console.log(this.foodDictionArray);
        console.log(this.foodAllArray);
    ...
    renderUncategorisedLogFoods() {
        ...
        for(let i = 0 ; i< foodPrelim.length ; ++i) {
            ...
            console.log(foodName, foodCounter);

            if(foodCounter > 0) foodsNotCategorised.push(foodName);
            // if(foodCounter === 0) this.deleteDictionaryFood(foodName);
        }
    ...
    //---------------- [RENDER HELPER 5.1] ------------------
    deleteDictionaryFood(food) {
        // ***** deletes from dictionary if user entered in log, then later deleted
        console.log('calling delete food from dictionary');
        console.log(food);
        this.preferences = StorageService.getPreferences();
        ...
        const indexToDelete = foodDictionary.indexOf(food);
        console.log(food)
        console.log(foodDictionary)
        console.log(indexToDelete)
        console.log(foodDictionary[indexToDelete]);
    ...
    saveFoodToDictionary(food) {
        ...
        this.preferences = StorageService.getPreferences();
        const foodDictionaryToSort = this.preferences.foodDictionary;
        
        foodDictionaryToSort.push(food);
        HelpString.sortNames(foodDictionaryToSort);

        this.preferences.foodDictionary = foodDictionaryToSort;

        console.log(this.preferences.foodDictionary);
    ...
    saveFoodModal() {
        ...
        const foodDictionArray = this.foodDictionary;
        ...
        if(modalType === 'edit-existing-food') {
            ...
            // cannot edit existing names - no change to dictionary
            ...
            this.saveFoodToDictionary(foodName);
        }
        ...
        const TEST = StorageService.getPreferences();
        console.log(TEST);
    ...
    attachDynamicEditFoodEventListeners(){
        ...
        for(const button of deleteButtons) {
            button.addEventListener('click', (event) => {
                const food = event.target.dataset.index;
                this.deleteFoodEntry(food);
                this.deleteDictionaryFood(food);
            });
        }
    ...
    deleteFoodEntry(food){
        // ***** also delete from food dictionary!
        console.log('calling delete food Array entry');
        const prefs = this.preferences;
